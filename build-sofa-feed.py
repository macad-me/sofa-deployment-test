...
def fetch_latest_os_version_info(os_type: str, os_version_name: str, gdmf_data: dict) -> dict:
    """Fetch the latest version information for the given OS type & version name,
    and determine 'ForkedLatest' if applicable."""
    print(f"Fetching latest: {os_type} {os_version_name}")
    os_versions_key = "macOS" if os_type == "macOS" else "iOS"  # Determine the key based on the OS type
    ...
    # Sort by SupportedDevices count (descending), then by PostingDate (latest date first)
    sorted_versions = sorted(
        filtered_versions,
        key=lambda version: (
            len(version.get("SupportedDevices", [])),  # Prioritize larger device counts
            datetime.strptime(version["PostingDate"], "%Y-%m-%d").timestamp()  # Use PostingDate for further sorting
        ),
        reverse=True
    )
    ...
    for version in sorted_versions[1:]:
        if len(version["SupportedDevices"]) != len(latest_version["SupportedDevices"]):
            ...
            break  # Add only one 'ForkedLatest' entry
