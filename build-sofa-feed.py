...
def write_data_to_json(feed_structure: dict, filename: str):
    ...
    for os_version in feed_structure["OSVersions"]:
        ...
        # Handle `ForkedLatest` entry
        if "ForkedLatest" in feed_structure:
            forked_latest = feed_structure["ForkedLatest"]
            forked_latest["ReleaseDate"] = format_iso_date(forked_latest.get("ReleaseDate", ""))
...
def OLD_write_data_to_json(feed_structure: dict, filename: str):
    """Writes the fully populated feed structure to JSON filename"""
    latest_versions = {}
